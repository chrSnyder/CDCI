...
titlePanel("Welcome to CDCI-SHiNE"),
  sidebarLayout(
    sidebarPanel(
      ...
      #radioButtons('scoreMethod','Method of Scoring' ,c("Absolute","Partial","True/False","All"))
      selectInput('scoreMethod','Method of Scoring' ,c("Absolute","Partial","True/False","All"))
      #radioButtons('quote','Quote',c(None='','Double'='"','Single'="'"),'"')
    ,
    actionButton('gradeNow','Grade Test using currently selected method')
    ,
    downloadLink('downloadScored','<Download Scored Test as CSV>')
    ,
    actionButton('sumStats','Display Summary Statistics')
    ,
    #radioButtons('plot','Plot',c("Average Score by Question", "Binary by Q (density)","Binary by Q (normal distribution)","Partial Credit by Q", "Boxplot","Binary by Q (compare normal and kernel densities)"))
    selectInput('plot','Plot',c("Binary by Q", "Binary by Q (density)","Binary by Q (normal distribution)","Partial Credit by Q", "Boxplot","Binary by Q (compare normal and kernel densities)"))
    ,
